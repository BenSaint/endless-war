...
async def instructions(cmd):
	...
	if cmd.message.channel.name != ewcfg.channel_slimeoidlab:
		...
		response += "\n\nThank you for choosing SlimeCorp."
...
async def growbody(cmd):
	...
	if slimeoid.life_state == 0:
		...
		slimeoid.body = "teardrop"
		slimeoid.head = "beast"
		slimeoid.legs = "legs"
		slimeoid.weapon = "blades"
		slimeoid.armor = "boneplates"
		slimeoid.special = "laser"
		slimeoid.name = "testbeast"
		slimeoid.atk = 1
		slimeoid.ai = "a"
		...
		response = "You mash buttons and just make whatever."

	# Send the response to the player.
	await cmd.client.edit_message(resp, ewutils.formatMessage(cmd.message.author, response))

async def Xgrowbody(cmd):
	resp = await start(cmd = cmd)
	user_data = EwUser(member = cmd.message.author)
	slimeoid = EwSlimeoid(member = cmd.message.author)
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a body type. Options are "
...
async def growhead(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a head type. Options are "
...
async def growlegs(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify means of locomotion. Options are "
...
async def growweapon(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a means of attack. Options are "
...
async def growarmor(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a method of protection. Options are "
...
async def growspecial(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a special attack type. Options are "
...
async def growbrain(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			brain != None
			response = "A cluster of neurons coagulates within your newly-forming slimeoid."
			response += " {}".format(brain.str_create)
			slimeoid.ai = brain.id_brain
			slimeoid.persist()
		else:
			response = "You must specify a brain structure. Choose from brain types A, B, C, D, E, F, or G. "
