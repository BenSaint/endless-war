...
async def instructions(cmd):
	...
	if cmd.message.channel.name != ewcfg.channel_slimeoidlab:
		...
		response += "\n\nAfter beginning incubation, you will need to use the console to adjust your Slimeoid's features while it is still forming. Use **!growbody**, **!growhead**, **!growlegs**, **!growweapon**, **!growarmor**, **!growspecial**, and **!growbrain** to see the options available and choose the appearance, abilities, and temperament of your Slimeoid. You will also need to give youe Slimeoid a name. Use **!nameslimeoid** followed by your desired name. These traits may be changed at any time before the incubation is completed."
		# Send the response to the player.
		...
		response += "\n\nSlimeoid research is ongoing, and the effects of a Slimeoid's physical makeup, brain structure, and attribute allocation on its abilities are a rapidly advancing field. Field studies into the effects of these variable on one-on-one Slimeoid battles are set to begin in the near future. In the meantime, report any unusual findings or behaviors to the Cop Killer and Rowdy Fucker, who have much fewer important things to spend their time on than SlimeCorp employees."
		response += "\n\nThank you for choosing SlimeCorp. :slimecorp:"
...
async def dissolveslimeoid(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if slimeoid.life_state == 1:
			...
			response = " {}".format(brain.str_dissolve)
...
async def growhead(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a head type. Choose an option from the buttons on the head console labelled A through F."
...
async def growlegs(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify means of locomotion. Choose an option from the buttons on the mobility console labelled A through D."
...
async def growweapon(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a means of attack. Choose an option from the buttons on the weapon console labelled A through D."
...
async def growarmor(cmd):
	resp = await start(cmd = cmd)
	armortype = ""
	all_armor = ["scales", "boneplates", "quantumfield", "formless"]
	user_data = EwUser(member = cmd.message.author)
	slimeoid = EwSlimeoid(member = cmd.message.author)
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a method of protection. Choose an option from the buttons on the armor console labelled A through D."
...
async def growspecial(cmd):
	...
	if slimeoid.life_state == 0:
		...
		if cmd.tokens_count > 1:
			...
			response = "You must specify a special attack type. Choose an option from the buttons on the special attack console labelled A through D."
